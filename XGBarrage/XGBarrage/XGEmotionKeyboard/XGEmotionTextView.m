...
#import "XGEmotionTextView.h"
#import "XGEmotion.h"
#import "XGEmotionAttachment.h"

@implementation XGEmotionTextView
...
- (void)appendEmotion:(XGEmotion *)emotion
{
    if (emotion.emoji) { // emoji表情
        ...
        // 创建一个带有图片表情的富文本
        XGEmotionAttachment *attach = [[XGEmotionAttachment alloc] init];
...
- (NSString *)realText
{
    ...
    [self.attributedText enumerateAttributesInRange:NSMakeRange(0, self.attributedText.length) options:0 usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *stop) {
        XGEmotionAttachment *attach = attrs[@"NSAttachment"];
